...
var MutationObserverCtor : any;
if (typeof (window as any).WebKitMutationObserver !== 'undefined')
  MutationObserverCtor = (window as any).WebKitMutationObserver;
else
...
class NodeMap<T> {
  ...
  private nodeId(node:Node) {
    var id = (node as any)[NodeMap.ID_PROP];
    if (!id)
      id = (node as any)[NodeMap.ID_PROP] = NodeMap.nextId_++;
    return id;
  }
...
function recordOldPrevious(change: ChildListChange, node:Node, previous:Node) {
        if (!node ||
...
class MutationProjection {
  ...
  getChanged(summary:Summary, selectors:Selector[], characterDataOnly:boolean) {
    ...
    for (var i = 0; i < this.entered.length; i++) {
      ...
      var matchable = this.matchabilityChange(node) as any;
...
function validateElementAttributes(attribs:string):string[] {
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    if ((lowerAttributes as any)[nameLower])
      throw Error('Invalid request option: observing multiple case variations of the same attribute is not supported.');

    (attributes as any)[name] = true;
    (lowerAttributes as any)[nameLower] = true;
  }
